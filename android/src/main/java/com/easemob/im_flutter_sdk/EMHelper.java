...
class EMHelper {
        ...
        try {
            ...
            switch(data_type){
                ...
                case 2:
                    ...
                    if(data_body.has("fileLength"))
                    {
                        fileLength = Long.parseLong(data_body.getString("fileLength"));
                    }
                    ...
                    if (videoFile.exists()){
                        ...
                        EMVideoMessageBody videoMessageBody = new EMVideoMessageBody(videoUrl,getThumbBitmap(videoUrl),videoDuration,fileLength);
    ...
    private static String getThumbBitmap(String videoPath){
        ...
        try {
            ...
            Bitmap ThumbBitmap = ThumbnailUtils.createVideoThumbnail(videoPath, MediaStore.Images.Thumbnails.MINI_KIND);
    ...
    static Map<String, Object> convertEMMessageBodyToStringMap(EMMessageBody mb) {
        ...
        if (mb instanceof EMTextMessageBody) {
            ...
            EMFileMessageBody.EMDownloadStatus dStatus = normalFileMessageBody.downloadStatus();
            int statusInt = 1;
            if(dStatus == EMFileMessageBody.EMDownloadStatus.DOWNLOADING)
            {
                statusInt = 0;
            }
            else if(dStatus == EMFileMessageBody.EMDownloadStatus.FAILED)
            {
                statusInt = 2;
            }
            else if(dStatus == EMFileMessageBody.EMDownloadStatus.PENDING)
            {
                statusInt = 3;
            }

            body.put("downloadStatus", Integer.valueOf(statusInt));
            ...
            EMFileMessageBody.EMDownloadStatus dStatus = imageMessageBody.downloadStatus();
            int statusInt = 1;
            if(dStatus == EMFileMessageBody.EMDownloadStatus.DOWNLOADING)
            {
                statusInt = 0;
            ...
            EMFileMessageBody.EMDownloadStatus dStatus = voiceMessageBody.downloadStatus();
            int statusInt = 1;
            if(dStatus == EMFileMessageBody.EMDownloadStatus.DOWNLOADING)
            {
                statusInt = 0;
            ...
            EMFileMessageBody.EMDownloadStatus dStatus = videoMessageBody.downloadStatus();
            int statusInt = 1;
            if(dStatus == EMFileMessageBody.EMDownloadStatus.DOWNLOADING)
            {
                statusInt = 0;
