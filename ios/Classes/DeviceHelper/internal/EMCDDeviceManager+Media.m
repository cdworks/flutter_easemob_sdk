...
- (void)asyncPlayingWithPath:(NSString *)aFilePath
    ...
    if (![fileManager fileExistsAtPath:wavFilePath]) {
        ...
        if (!covertRet) {
            if (completon) {
                completon([NSError errorWithDomain:@"格式化音频格式失败"                                              code:EMErrorFileTypeConvertionFailure
                                          userInfo:nil]);
            }
            return ;
        }
...
- (void)asyncStartRecordingWithFileName:(NSString *)fileName
    ...
    if ([self isRecording]) {
        if (completion) {
            error = [NSError errorWithDomain:@"已经有录音了"
    ...
    if (!fileName || [fileName length] == 0) {
        error = [NSError errorWithDomain:@"找不到文件"
                                    code:-1
                                userInfo:nil];
        completion(error);
        return ;
    ...
    if (isNeedActive) {
        BOOL success = [audioSession setActive:isActive
                                   withOptions:AVAudioSessionSetActiveOptionNotifyOthersOnDeactivation
                                         error:&error];
        if(!success || error){
            error = [NSError errorWithDomain:@"初始化音视频失败"
                                        code:-1
                                    userInfo:nil];
            return error;
        }
