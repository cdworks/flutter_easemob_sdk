...
//         message.status = EMMessage.from(response['message']).status;
  ...
  void resendMessage(EMMessage message,
      { onSuccess(),
        ...
        onError(int errorCode, String desc)}){
    Future<Map> result = _emChatManagerChannel.invokeMethod(
        EMSDKMethod.resendMessage, message.toDataMap());
    result.then((response){
      if (response["success"]) {
//        message.msgId = response['ServerMsgId'];
        var respnseMsg = response['message'];
...
//         message.status = EMMessage.from(response['message']).status;
  ...
  void saveMessage(EMMessage message) {
    _emChatManagerChannel.invokeMethod(
        EMSDKMethod.saveMessage, message.toDataMap());
  }
  ...
  Future<bool> updateMessage(EMMessage message) async {
    Map<String, dynamic> result = await _emChatManagerChannel.invokeMethod(
        ...
        message.toDataMap()); //TODO: only message id needed?
    if (result['success']) {
  ...
  void downloadAttachment(EMMessage message) {
    _emChatManagerChannel.invokeMethod(
        EMSDKMethod.downloadAttachment, message.toDataMap());
  }

  /// @nodoc 下载缩略图 [message].
  void downloadThumbnail(EMMessage message) {
    _emChatManagerChannel.invokeMethod(
        EMSDKMethod.downloadThumbnail, message.toDataMap());
  }
  ...
  void removeMessageStatusListener(EMMessageStatus listener) {
    assert(listener != null);
